...
async function createRandomFakeUsers() {
    ...
    return Promise.all(new Array(config.get('application.numberOfRandomFakeUsers')).fill(0).map(() => models.User.create({
...
async function createProducts() {
    ...
    return Promise.all(products.map(({ reviews = [], useForChristmasSpecialChallenge = false, urlForProductTamperingChallenge = false, fileForRetrieveBlueprintChallenge = false, ...product }) => models.Product.create(product).catch((err) => {
